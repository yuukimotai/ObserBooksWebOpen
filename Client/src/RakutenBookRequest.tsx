    ...
    setList(
      books.map((book, index) => (
        <section className="m-2 w-52">
          ...
          <img className="mx-auto" src={book["Item"].largeImageUrl} />
          <p className="flex flex-row">
            <button className="text-xs" onClick={() => AddMyBook(book)}>マイブック</button>
            <button className="text-xs" onClick={() => AddAttentionBook(book)}>気になる！</button>
          </p>
        </section>
      ))
    );
